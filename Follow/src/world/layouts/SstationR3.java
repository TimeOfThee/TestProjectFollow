...
public class SstationR3 extends Layout{
	
	int clor=0;
	boolean tog=true;
	
	public SstationR3(Handler handler,int width,int height) {
	...
	public void render(Graphics g,float cx,float cy) {
		...
		g.fillRect((int)(0+cx), (int)(0+cy), width, height);
	...
	public void enterFrom(int dir) {//updoleri
		...
		case 1://do
			...
			break;
		case 2://le
			spawnPlayer(1,handler.getPlayer().getY()-3+ (height-disGroundL)-(conLe.getHeight()-conLe.getDisGround(true)) );
			break;
		case 3://ri
			spawnPlayer(width-1-handler.getPlayer().getWidth(),handler.getPlayer().getY()-3+ (height-disGroundR)-(conRi.getHeight()-conRi.getDisGround(false)));
			break;
		default:
